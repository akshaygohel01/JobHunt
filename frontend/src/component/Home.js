...
const JobTile = (props) => {
  ...
  const handleApply = () => {
    ...
    formData.append("gender", gender); // Assuming 'gender' is the variable storing the selected gender value
    ...
    axios
      .post(`${apiList.jobs}/${job._id}/applications`, formData, {
        headers: {
      ...
      .then((response) => {
        ...
        const appliedJobs =
          JSON.parse(localStorage.getItem(`appliedJobs_${userId}`)) || [];
        localStorage.setItem(
          `appliedJobs_${userId}`,
          JSON.stringify([...appliedJobs, job._id])
        );
      })
      .catch((err) => {
      ...
      <Grid container>
        ...
        <Grid item xs={3}>
          <Button
            ...
            disabled={userType() === "recruiter" || hasApplied(job._id)}
          >
      ...
      <Modal open={open} onClose={handleClose} className={classes.popupDialog}>
        ...
        >
          ...
          <div style={{ display:"flex", flexDirection: "row", width:"100%"}}>
            <label style={{marginRight: "10px"}}>Upload Resume:</label>
            <input
              type="file"
    ...
    <>
      ...
      >
        ...
        >
          {jobs.length > 0 ? (
            jobs.map((job) => {
              return <JobTile job={job} applied={applications} />;
            })
          ) : (
            <Typography
              variant="h5"
